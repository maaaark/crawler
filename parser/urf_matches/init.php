...
if(isset($_GET["normal_parser"])){
...
} elseif(isset($_GET["analyse_parser"])){
   if(isset($_GET["run"])){
      require_once 'parser/urf_matches/parser_analyse.init.php';
   } else {
      $template = new template;
      $template->load("analyse_parser");
      $template->assign("SITE_TITLE", "URF-Matches Parser Analysen");
      $tmpl = $template->display(true);
      $tmpl = $template->operators();
      echo $tmpl;
   }
   ...
   $nums_analysed = $GLOBALS["db"]->num_rows($GLOBALS["db"]->query("SELECT * FROM urf_matchIDs WHERE status = '1'"));
   
   $template = new template;
   $template->load("index");
   $template->assign("SITE_TITLE", "URF-Matches Parser");
   $template->assign("NUMS_ALL", number_format($nums, 0, ",", "."));
   $template->assign("NUMS_ANALYSED", number_format($nums_analysed, 0, ",", "."));
