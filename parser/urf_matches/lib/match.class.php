...
class Match {
	...
	public function analyse(){
		...
		if(isset($json["participants"])){
			...
			// Match speichern
			$GLOBALS["db"]->query("INSERT INTO lol_league_parser_matches_nurf SET id = '".trim($this->match_id)."', patch = '".GAME_VERSION."'");
		}
	...
	private function save($array){
		$check = $GLOBALS["db"]->fetch_array($GLOBALS["db"]->query("SELECT * FROM lol_champions_stats_nurf WHERE champion = '".$array["champion"]."' AND patch = '".$GLOBALS["db"]->real_escape_string(GAME_VERSION)."'"));
		
		if(isset($check["id"]) && $check["id"] > 0){
			$sql 		 = "UPDATE lol_champions_stats_nurf SET ";
			$sql_type = "update";
		} else {
			$sql 		 = "INSERT INTO lol_champions_stats_nurf SET champion = '".$array["champion"]."', patch = '".$GLOBALS["db"]->real_escape_string(GAME_VERSION)."', ";
			$sql_type = "insert";
		}
	...
	private function addBan($champion_id, $pick){
		$check = $GLOBALS["db"]->fetch_array($GLOBALS["db"]->query("SELECT * FROM lol_champions_stats_nurf_bans WHERE champion = '".$GLOBALS["db"]->real_escape_string($champion_id)."' AND 
                                                                                                                    patch    = '".$GLOBALS["db"]->real_escape_string(GAME_VERSION)."'"));
		if(isset($check["id"]) && $check["id"] > 0){
			$pick_count 	= $check["ban_".trim($pick)] + 1;
			$new_ban_count = $check["bans"] + 1;
			$sql 				= "UPDATE lol_champions_stats_nurf_bans SET bans = '".$new_ban_count."', ban_".trim($pick)." = '".$pick_count."' WHERE id = '".$check["id"]."'";
		} else {
			$sql = "INSERT INTO lol_champions_stats_nurf_bans SET champion = '".trim($champion_id)."', bans = '1', ban_".trim($pick)." = '1', patch = '".$GLOBALS["db"]->real_escape_string(GAME_VERSION)."'";
		}
